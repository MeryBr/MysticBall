...
function Orb({ answer, broken }: { answer: string; broken: boolean }) {
    ...
    return (
        <mesh>
            ...
            <shaderMaterial
                ...
                fragmentShader={broken ? brokenFragmentShader : normalFragmentShader}
                transparent={true}
                depthWrite={false}
            />
            {answer && (
                <Html center zIndexRange={[100, 0]}>
                    <div className="answer-inside-ball">
                        {answer}
                    </div>
                </Html>
            )}
...
export default function MagicSphere({
    ...
    return (
        <div
            className={`magic-sphere-wrapper ${broken ? 'broken' : ''}`}
            style={{
                width: 360,
                height: 360,
                margin: '0 auto',
        ...
        >
            {broken && <div className="broken-overlay" />}

            {broken && <div className="lightning-flash" />}
            ...
            >
                ...
                <Orb answer={answer} broken={broken} />
            </Canvas>
        </div>
    );
